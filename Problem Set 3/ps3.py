...
def get_word_score(word, n):
    ...
    first_component = 0
    for x in word:
        first_component += SCRABBLE_LETTER_VALUES.get(x.lower())
    
    calc = (7 * len(word)) - 3 * (n - len(word))
    if calc < 1:
        second_component = 1
    else:
        second_component = calc
...
def is_valid_word(word, hand, word_list):
    ...
    if '*' in word:
        for vowel in VOWELS:
            wild = word.replace('*', vowel)
            if wild in word_list:
                return True
        return False
    ...
    * The hand finishes when there are no more unused letters.
    ...
    while handlen > 0:
        ...
        if is_valid_word(word, hand, word_list):
            ...
            print(f'"{word}" earned {get_word_score(word, handlen)} points. Total: {score} points\n')
        else:
...
def substitute_hand(hand, letter):
    ...
    taken = list(clone_hand.keys())
    count = clone_hand.pop(letter, 0)
    ...
    clone_hand[random.choice(available_letters)] = count
